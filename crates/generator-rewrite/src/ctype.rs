//! C type classification, primitive name mapping, and C→Rust type conversion.
...
/// Convert a full C type to its Rust string representation (raw C-layout).
///
/// For a structured `RustType` equivalent, use `build::type_conv::resolve_ctype`.
pub fn ctype_to_rust_type(analysis: &Analysis, ty: &CType, lifetime: Option<&str>) -> String {
    match ty {
        ...
        CType::Array { element, len } => {
            ...
            if is_char {
                match len {
                    ...
                    CArrayLen::Literal(len) => format!("ArrayCStr<{{ {len} }}>"),
                }
            } else {
                ...
                match len {
                    ...
                    CArrayLen::Literal(len) => format!("[{element_ty}; {len}]"),
                }
            }
        }
        CType::Func { .. } => todo!(),
