...
impl FuncPointerParam {
    fn from_node(node: Node) -> FuncPointerParam {
        FuncPointerParam {
    ...
    fn from_node(node: Node) -> FuncPointer {
        ...
        FuncPointer {
            ...
            params: node
                ...
                .map(FuncPointerParam::from_node)
                .collect(),
            requires: attribute(node, "requires"),
        }
    }
...
impl BitMask {
    fn from_node(node: Node, api: &str) -> BitMask {
        let mut value = BitMask {
            ...
            values: Vec::new(),
            aliases: Vec::new(),
            bitwidth: attribute(node, "bitwidth").map(|value| value.parse().unwrap()),
        };
