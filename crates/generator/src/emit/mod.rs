...
use crate::{
    analysis::Analysis,
    model::{ApiConstantDef, BasetypeDef, TypeAliasDef},
    write_doc_link, xml,
};
...
pub fn generate_api_constants(
    ...
    for constant in api_constants {
        let m = crate::model::build_api_constant(constant);
...
pub fn generate_basetypes(file: &mut impl Write, basetypes: &[&xml::BaseType]) -> Result<()> {
    for ty in basetypes {
        let m = crate::model::build_basetype(ty);
...
pub fn generate_type_aliases(
    ...
    for alias in type_aliases {
        let m = crate::model::build_type_alias(analysis, alias);
